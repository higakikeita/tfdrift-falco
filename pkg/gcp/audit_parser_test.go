...
func TestAuditParser_isRelevantEvent(t *testing.T) {
	...
	tests := []struct {
		...
		{"Dataproc Cluster Create", "google.cloud.dataproc.v1.ClusterController.CreateCluster", true},
		{"Dataflow Job Create", "google.dataflow.v1b3.JobsV1Beta3.CreateJob", true},
		{"Spanner Instance Create", "google.spanner.admin.instance.v1.InstanceAdmin.CreateInstance", true},
		{"Spanner Database Create", "google.spanner.admin.database.v1.DatabaseAdmin.CreateDatabase", true},
		{"Firestore Index Create", "google.firestore.admin.v1.FirestoreAdmin.CreateIndex", true},
		{"Firestore Database Create", "google.firestore.admin.v1.FirestoreAdmin.CreateDatabase", true},
		{"Redis Instance Create", "google.cloud.redis.v1.CloudRedis.CreateInstance", true},
		{"DNS Managed Zone Create", "dns.managedZones.create", true},
		{"DNS Record Create", "dns.changes.create", true},
		{"Logging Sink Create", "google.logging.v2.ConfigServiceV2.CreateSink", true},
		{"Monitoring Alert Policy Create", "google.monitoring.v3.AlertPolicyService.CreateAlertPolicy", true},
		{"App Engine Service Update", "google.appengine.v1.Services.UpdateService", true},
		{"Cloud Armor Policy Insert", "compute.securityPolicies.insert", true},
		{"Service Perimeter Create", "google.identity.accesscontextmanager.v1.AccessContextManager.CreateServicePerimeter", true},
		{"Artifact Registry Repository Create", "google.devtools.artifactregistry.v1.ArtifactRegistry.CreateRepository", true},
		{"Cloud Composer Environment Create", "google.cloud.orchestration.airflow.service.v1.Environments.CreateEnvironment", true},
		{"Cloud Tasks Queue Create", "google.cloud.tasks.v2.CloudTasks.CreateQueue", true},
		{"Cloud Scheduler Job Create", "google.cloud.scheduler.v1.CloudScheduler.CreateJob", true},
		{"Cloud Build Trigger Create", "google.devtools.cloudbuild.v1.CloudBuild.CreateBuildTrigger", true},
		{"Cloud Endpoints Service Create", "google.api.servicemanagement.v1.ServiceManager.CreateService", true},
		{"Vertex AI Endpoint Create", "google.cloud.aiplatform.v1.EndpointService.CreateEndpoint", true},
...
func TestAuditParser_extractChanges(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			map[string]string{"gcp.request": `{"statements": ["CREATE TABLE ..."]}`},
			[]string{"ddl", "_raw_request"},
		},
		{
			"IAM Policy Update",
			"google.monitoring.v3.AlertPolicyService.UpdateAlertPolicy",
			map[string]string{"gcp.request": `{"bindings": [...]}`},
			[]string{"_action", "_update_request", "_raw_request"},
		},
		{
			"DNS Record Create",
			"dns.changes.create",
			map[string]string{"gcp.request": `{"rrdata": [...]}`},
			[]string{"rrdata", "_raw_request"},
		},
		{
			"Security Policy Rule Update",
			"compute.securityPolicies.addRule",
			map[string]string{"gcp.request": `{"rules": [...]}`},
			[]string{"rules", "_raw_request"},
		},
		{
			"Pub/Sub Modify Push Config",
			"google.pubsub.v1.Subscriber.ModifyPushConfig",
			map[string]string{"gcp.request": `{"pushConfig": {...}}`},
			[]string{"push_config", "_raw_request"},
		},
		{
			"Dataproc Cluster Create",
			"google.cloud.dataproc.v1.ClusterController.CreateCluster",
			map[string]string{"gcp.response": `{"id": "cluster-1"}`},
			[]string{"_action", "_created_resource", "_raw_response"},
		},
		{
...
func TestAuditParser_Parse_AllServiceTypes(t *testing.T) {
	...
	services := []struct {
		...
		{"dataproc.googleapis.com", "google.cloud.dataproc.v1.ClusterController.CreateCluster", "projects/p/regions/r/clusters/c", "google_dataproc_cluster"},
		{"spanner.googleapis.com", "google.spanner.admin.instance.v1.InstanceAdmin.CreateInstance", "projects/p/instances/i", "google_spanner_instance"},
		{"firestore.googleapis.com", "google.firestore.admin.v1.FirestoreAdmin.CreateDatabase", "projects/p/databases/d", "google_firestore_database"},
		{"redis.googleapis.com", "google.cloud.redis.v1.CloudRedis.CreateInstance", "projects/p/locations/l/instances/i", "google_redis_instance"},
		{"dns.googleapis.com", "dns.managedZones.create", "projects/p/managedZones/z", "google_dns_managed_zone"},
		{"logging.googleapis.com", "google.logging.v2.ConfigServiceV2.CreateSink", "projects/p/sinks/s", "google_logging_project_sink"},
		{"monitoring.googleapis.com", "google.monitoring.v3.AlertPolicyService.CreateAlertPolicy", "projects/p/alertPolicies/a", "google_monitoring_alert_policy"},
		{"appengine.googleapis.com", "google.appengine.v1.Services.UpdateService", "apps/p/services/s", "google_app_engine_service_split_traffic"},
		{"compute.googleapis.com", "compute.securityPolicies.insert", "projects/p/global/securityPolicies/s", "google_compute_security_policy"},
		{"accesscontextmanager.googleapis.com", "google.identity.accesscontextmanager.v1.AccessContextManager.CreateServicePerimeter", "accessPolicies/p/servicePerimeters/s", "google_access_context_manager_service_perimeter"},
		{"artifactregistry.googleapis.com", "google.devtools.artifactregistry.v1.ArtifactRegistry.CreateRepository", "projects/p/locations/l/repositories/r", "google_artifact_registry_repository"},
		{"composer.googleapis.com", "google.cloud.orchestration.airflow.service.v1.Environments.CreateEnvironment", "projects/p/locations/l/environments/e", "google_composer_environment"},
		{"cloudtasks.googleapis.com", "google.cloud.tasks.v2.CloudTasks.CreateQueue", "projects/p/locations/l/queues/q", "google_cloud_tasks_queue"},
		{"cloudscheduler.googleapis.com", "google.cloud.scheduler.v1.CloudScheduler.CreateJob", "projects/p/locations/l/jobs/j", "google_cloud_scheduler_job"},
		{"cloudbuild.googleapis.com", "google.devtools.cloudbuild.v1.CloudBuild.CreateBuildTrigger", "projects/p/triggers/t", "google_cloudbuild_trigger"},
		{"servicemanagement.googleapis.com", "google.api.servicemanagement.v1.ServiceManager.CreateService", "projects/p/services/s", "google_endpoints_service"},
		{"aiplatform.googleapis.com", "google.cloud.aiplatform.v1.EndpointService.CreateEndpoint", "projects/p/locations/l/endpoints/e", "google_vertex_ai_endpoint"},
	}
