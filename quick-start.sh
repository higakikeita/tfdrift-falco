...
echo -e "${BOLD}🚀 driftwire Quick Start (v${VERSION})${NC}"
echo ""
echo "This script will set up driftwire with Falco in Docker."
echo ""
...
echo ""
echo -e "${BOLD}🔧 driftwire Configuration${NC}"
echo ""
...
# Create driftwire configuration
echo ""
echo -e "${YELLOW}Creating driftwire configuration...${NC}"
cat > config.yaml <<EOF
# driftwire Configuration (Auto-generated by quick-start.sh)
...
echo -e "${GREEN}✅ driftwire configuration created${NC}"

# Create .env file
echo ""
echo -e "${YELLOW}Creating .env file...${NC}"
cat > .env <<EOF
# driftwire Environment Variables
...
echo ""
echo -e "${YELLOW}${BOLD}⚠️  SECURITY: the API server has NO built-in authentication and Falco gRPC${NC}"
echo -e "${YELLOW}${BOLD}   defaults to plaintext. docker-compose publishes ports on 127.0.0.1 only.${NC}"
echo -e "${YELLOW}${BOLD}   Keep it on localhost / a trusted network. Do NOT expose it to the public${NC}"
echo -e "${YELLOW}${BOLD}   internet without putting authentication + TLS in front. See SECURITY.md.${NC}"
echo ""
echo -e "${BOLD}🚀 Start the local UI/API demo:${NC}"
echo ""
echo -e "  ${BOLD}docker compose up -d${NC}"
echo ""
echo -e "${YELLOW}ℹ️  This default stack is a local UI/API demo — it does NOT enable real-time${NC}"
echo -e "${YELLOW}   CloudTrail detection (Falco ships without the CloudTrail plugin here).${NC}"
echo -e "${YELLOW}   To enable real-time drift (needs the Falco CloudTrail plugin + AWS), see${NC}"
echo -e "${YELLOW}   docs/complete-setup-guide.md.${NC}"
echo ""
echo -e "${BOLD}📊 View logs:${NC}"
echo ""
echo "  docker compose logs -f driftwire"
...
echo "  4. Read full documentation: https://github.com/higakikeita/driftwire"
echo ""
