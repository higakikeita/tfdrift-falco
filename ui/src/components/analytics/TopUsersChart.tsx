...
interface TopUsersChartProps {
  data: { user: string; events: number }[];
}

export function TopUsersChart({ data }: TopUsersChartProps) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5">
      <h3 className="text-sm font-semibold text-slate-700 mb-4">Top Drift-Causing Users</h3>
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={data} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 0 }}>
          ...
          <YAxis type="category" dataKey="user" tick={{ fontSize: 11 }} stroke="#94a3b8" width={130} />
          <Tooltip contentStyle={{ borderRadius: 8, fontSize: 13, border: '1px solid #e2e8f0' }} />
          <Bar dataKey="events" fill="#f59e0b" radius={[0, 4, 4, 0]} barSize={18} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
