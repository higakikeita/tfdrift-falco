...
This release brings comprehensive Azure support to driftwire, completing the multi-cloud trifecta (AWS + GCP + Azure). Azure now implements the full Provider interface including resource discovery and state comparison, alongside a new Azure Blob Storage Terraform backend and enhanced real-time WebSocket/SSE capabilities.
