...
driftwire test --state=terraform.tfstate --events=cloudtrail.json

# Generate service coverage report
driftwire coverage --service=all

# Export drift history
driftwire export --format=csv --days=30
```

### Terraform Provider
```hcl
resource "driftwire_monitor" "api_gateway" {
  service = "apigateway"
...
**Submit feedback:** https://github.com/higakikeita/driftwire/discussions
...
- [GitHub Milestones](https://github.com/higakikeita/driftwire/milestones)
- [Project Board](https://github.com/higakikeita/driftwire/projects)
